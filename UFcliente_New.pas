...
begin
 MessageDlg('O campo Razão Social não pode ficar vazio',mtError,[mbOK],0);
 DBEdit4.SetFocus;
 exit;
end;
...
begin
 MessageDlg('Informe o município do cliente',mtError,[mbOK],0);
 DBLookCid.SetFocus;
 exit;
end;
...
begin
 MessageDlg('É necessário informar o código do cliente',mtError,[mbOK],0);
 DBEdit1.SetFocus;
...
Banco.TBClientesUF.Value         := ComboBox1.Text;
    ...
    begin
      ...
      MessageDlg('Cliente já registrado',mtError,[mbok],0);
      end
        else
         begin
          Banco.Create_Erro(e.ClassName,e.Message+#13+#13+#13+'REGISTRANDO CLIENTE TABLEA TBClientes');
          MessageDlg('Não foi possível registrar o cliente, Mensagem original'+#13+e.Message,mtError,[mbOK],0)
        end;
